...
ed_1516_all <- demo_1516 %>% 
  ...
  mutate(
    ...
    htn = if_else((htn_bp == 1 | htn_meds == 1), true = 1, false =  0))%>%
    filter(DMDEDUC2 != 9) %>% 
# indicator plus restricting:
    mutate(
      ed_all_i = 1
    ) %>% 
  select(SEQN, ed_all_i)


ed_1516_htn <- demo_1516 %>% 
  filter(WTMEC2YR > 0) %>% 
  filter(RIDAGEYR >= 20) %>% 
  filter(RIDEXPRG != 1 | is.na(RIDEXPRG)) %>% 
  left_join(bpm_1516, by = "SEQN") %>% 
...
# indicator plus restricting:
  ...
  select(SEQN, ed_htn_i)



inc_1516_all <- demo_1516 %>% 
  filter(WTMEC2YR > 0) %>% 
  filter(RIDAGEYR >= 20) %>% 
  filter(RIDEXPRG != 1 | is.na(RIDEXPRG)) %>% 
  left_join(bpm_1516, by = "SEQN") %>% 
...
# indicator plus restricting:
  ...
  select(SEQN, inc_all_i)



inc_1516_htn <- demo_1516 %>% 
  filter(WTMEC2YR > 0) %>% 
  filter(RIDAGEYR >= 20) %>% 
  filter(RIDEXPRG != 1 | is.na(RIDEXPRG)) %>% 
  left_join(bpm_1516, by = "SEQN") %>% 
